...
function Option(props) {
  const startWithCode = props.children.indexOf('[');
  const endWithCode = props.children.indexOf(']');
  const startWithoutCode = props.children.indexOf('-');
  const code = props.children.slice(startWithCode, endWithCode + 1);
  return (
    <div
    ...
    >
      <p>
        {code.length > 1 && (
          <>
            <span style={{ color: 'darkgreen', fontWeight: 'bold' }}>
              {props.children.slice(0, startWithCode)}
            </span>
            <strong>{code}</strong>
            {props.children.slice(endWithCode + 1, props.children.length)}
          </>
        )}
        ...
        {code.length < 1 && (
          <>
            <span style={{ color: 'darkgreen', fontWeight: 'bold' }}>
              {props.children.slice(0, startWithoutCode)}
            </span>
            {props.children.slice(startWithoutCode, props.children.length)}
          </>
        )}
      </p>
    </div>
...
export default withStyles(styles, { withTheme: true })(CustomSelectComponent);
